...
`user.profile` is the structured user profile envelope — see §5a. It's
`null` for users with no preference row yet (brand-new accounts before
onboarding); populated otherwise.

### `user.profile` envelope (§5a)

A consumer-ready, structured representation of how the user wants to be
served. Generated deterministically from preferences + activities by
`prompt_builder.py`. Stored on `user_preference.profile_json`. Lazy
regenerated by mobius-user when stale.

```json
{
  "preferred_name": "Sarah",
  "tasks": [
    {"code": "verify_eligibility", "label": "Verify eligibility"},
    {"code": "submit_claims", "label": "Submit claims"}
  ],
  "communication": {
    "tone": "professional",
  ...
  "autonomy": {
    "routine_tasks": "automatic",
    "sensitive_tasks": "confirm_first"
  },
  "timezone": "America/New_York",
  "rendered_prompt": "The user's name is Sarah. They focus on eligibility verification and claims submission. Use a professional tone — clear, direct, no slang. Assume regular AI experience — explain anything unusual but skip the basics. For routine tasks, do them automatically without confirming. For sensitive tasks (billing, PHI, anything irreversible), show your plan and wait for confirmation.",
  "version": 1,
  "generated_at": "2026-05-06T13:42:00Z"
}
```

**Lifecycle**:
- Generated on `PUT /api/v1/auth/onboarding` (eager, in same transaction).
- Re-generated on `PUT /api/v1/auth/preferences` (eager).
- Lazy-regenerated by `GET /api/v1/auth/me` and the `_issue_session_for_user` path (login / register / google) when `version != current template version`. Pure-Python build, no LLM, safe inline.
- `null` when `user_preference` row doesn't exist yet (brand new user before onboarding).

**How to use it (recommended)**:
- For LLM system prompts: splice `user.profile.rendered_prompt` directly into the system message. One-line drop-in.
- For UI / programmatic logic: read the structured fields (`tasks`, `autonomy`, etc.) and react accordingly. Example: chat shows a "Confirm action" button when `autonomy.sensitive_tasks == "confirm_first"`.
- For both: read `version` so you can detect format changes and re-cache if needed.

**Versioning**:
- The `version` field is tied to the rendered prompt language and schema, not the auth API. When the template author bumps it, mobius-user lazily regenerates on next read.
- A consumer that caches `profile` should also cache `version`; on mismatch, drop cache and re-read.
